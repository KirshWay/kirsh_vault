...
export default function OtherPage() {
  ...
  const loadItems = async () => {
    setIsLoading(true);
    try {
      const otherItems = await db.getItemsByCategory('other');
      setItems(otherItems);
    } catch (error) {
  ...
  return (
    <div className="container mx-auto p-4 max-w-4xl">
      <header className="mb-8">
        <h1 className="text-3xl font-bold mb-2">Other Items</h1>
        <p className="text-muted-foreground">
          Manage your other collectible items
        </p>
      </header>
